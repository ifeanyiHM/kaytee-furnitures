...
import { AdminInquiryActions } from "@/components/admin/InquiryActions";
...
export default async function AdminInquiriesPage({
  ...
  const {
    ...
    total,
    pages,
  } = await adminGetInquiries(page, 20, sp.status).catch(() => ({
    data: [],
    total: 0,
    pages: 0,
  }));
  ...
  return (
    <div className="p-6 lg:p-8">
          ...
          <Link
            ...
            className={`px-4 py-1.5 rounded-full font-sans text-sm transition-colors capitalize ${
              sp.status === s || (!sp.status && !s)
                ...
                : "bg-white border border-sand-200 text-charcoal-muted hover:text-charcoal"
            }`}
          >
            {s ? s.toLowerCase() : "All"}
          </Link>
      ...
      <div className="space-y-3">
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`font-sans text-xs px-2 py-0.5 rounded-full ${statusColors[inquiry.status]}`}
                >
            ...
            <p className="font-sans text-sm text-charcoal-muted line-clamp-2 mb-3">
              {inquiry.message}
            </p>
            {inquiry.adminReply && (
              <div className="bg-brand-50 border border-brand-100 rounded-lg p-3 mb-3">
                <p className="font-sans text-xs text-brand-600 font-medium mb-1">
                  Your reply
                </p>
        ...
        {inquiries.length === 0 && (
          <div className="text-center py-12 bg-white border border-sand-200 rounded-xl font-sans text-charcoal-muted">
            No inquiries found.
          </div>
        )}
      </div>
