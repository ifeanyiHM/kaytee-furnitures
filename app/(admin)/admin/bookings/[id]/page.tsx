...
import { AdminBookingStatusForm } from "@/components/admin/BookingStatusForm";
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        <Link
          href="/admin/bookings"
          className="font-sans text-sm text-charcoal-muted hover:text-charcoal"
        >
          ← Bookings
        </Link>
      </div>
          ...
          <div className="bg-white border border-sand-200 rounded-xl p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              {[
                ...
                ["Space size", booking.spaceSize],
                ["Timeline", booking.timeline],
              ]
                .filter(([, v]) => v)
                .map(([k, v]) => (
                  <div key={k}>
                    <p className="font-sans text-xs text-charcoal-muted uppercase tracking-wider mb-0.5">
                      {k}
                    </p>
            ...
            <div>
              <p className="font-sans text-xs text-charcoal-muted uppercase tracking-wider mb-2">
                Project description
              </p>
              <p className="font-sans text-sm text-charcoal leading-relaxed bg-sand-50 rounded-lg p-4">
                {booking.description}
              </p>
            </div>
          </div>
