...
export const metadata: Metadata = {
  title: "Shop",
  description: "Browse our curated collection of premium furniture and décor.",
};
    ...
    <>
      <section className="pt-32 pb-8 bg-charcoal">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-display text-[clamp(2.5rem,5vw,4rem)] text-white">
            Shop furniture &amp; décor
          </h1>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-8">
          <div className="flex gap-2 flex-wrap">
            <Link
              href="/shop"
              className={`px-4 py-1.5 rounded-full font-sans text-sm transition-colors ${!sp.category ? "bg-brand-600 text-white" : "bg-sand-100 text-charcoal-muted hover:text-charcoal"}`}
            >
              All
            </Link>
            {(categories as { _id: string; name: string; slug: string }[]).map(
              (cat) => (
                <Link
                  key={cat._id}
          ...
          <form className="flex items-center gap-3">
            ...
            <button
              type="submit"
              className="h-9 px-4 bg-brand-600 text-white rounded font-sans text-sm hover:bg-brand-800 transition-colors"
            >
              Filter
            </button>
          </form>
