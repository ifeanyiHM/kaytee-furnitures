        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-3">
            <div className="relative aspect-square rounded-xl overflow-hidden bg-sand-100">
              ...
              />
              {discount && (
                <span className="absolute top-4 left-4 bg-brand-600 text-white text-sm font-medium px-3 py-1 rounded">
                  -{discount}%
                </span>
              )}
            </div>
            {product.images.length > 1 && (
              <div className="grid grid-cols-4 gap-3">
                {product.images.slice(0, 4).map((img, i) => (
                  <div
                    key={i}
                    className="relative aspect-square rounded-lg overflow-hidden bg-sand-100"
                  >
                    <Image
                      src={img}
          ...
          <div>
            ...
            <h1 className="font-display text-[clamp(2rem,4vw,3rem)] text-charcoal leading-tight mb-4">
              {product.name}
            </h1>
            ...
            <p className="font-sans text-charcoal-muted leading-relaxed mb-8">
              {product.description}
            </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2 flex-wrap">
                  {product.colors.map((c) => (
                    <span
                      key={c}
                      className="px-3 py-1 border border-sand-200 rounded-full font-sans text-sm text-charcoal"
                    >
                      {c}
                    </span>
                  ))}
              ...
              <div className="mt-8 pt-8 border-t border-sand-200">
                ...
                <div className="flex gap-2 flex-wrap">
                  {product.materials.map((m) => (
                    <span
                      key={m}
                      className="font-sans text-xs bg-sand-100 text-charcoal-muted px-2.5 py-1 rounded"
                    >
                      {m}
                    </span>
                  ))}
