...
export default async function CheckoutPage() {
  ...
  const subtotal = items.reduce(
    (s: number, i) => s + (i.productId?.price || 0) * i.quantity,
    0,
  );
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-xl border border-sand-200 p-6 h-fit">
                ...
                {items.map((item) => (
                  <div key={item.productId?._id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-sans text-sm text-charcoal line-clamp-1">
                        {item.productId?.name}
                      </p>
                    ...
                    <p className="font-sans text-sm text-charcoal shrink-0">
                      {formatCurrency(
              ...
              <div className="border-t border-sand-200 pt-4 space-y-2 font-sans text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-charcoal-muted">VAT (7.5%)</span>
