...
import type { ActionResult, BookingType, ServiceType, PaginatedResult } from "@/types";
...
export async function createBooking(data: unknown): Promise<ActionResult> {
  ...
  await sendBookingConfirmation({ firstName: parsed.data.firstName, email: parsed.data.email, serviceName: service.name });
...
export async function getUserBookings(): Promise<BookingType[]> {
  ...
  const bookings = await Booking.find({ userId: session.user.id })
    .populate("serviceId", "name slug").sort({ createdAt: -1 }).lean();
  return serialize(bookings) as BookingType[];
}

export async function adminGetBookings(page = 1, limit = 20, status?: string): Promise<PaginatedResult<BookingType>> {
  const session = await auth();
  ...
  const [data, total] = await Promise.all([
    Booking.find(query as any).populate("serviceId", "name").sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Booking.countDocuments(query as any),
  ]);
  return { data: serialize(data) as BookingType[], total, page, pages: Math.ceil(total / limit) };
}

export async function updateBookingStatus(id: string, status: string, adminNotes?: string): Promise<ActionResult> {
  const session = await auth();
  if ((session?.user as { role?: string })?.role !== "ADMIN") return { success: false, error: "Unauthorized" };

  await connectDB();
  await Booking.findByIdAndUpdate(id, { status, ...(adminNotes && { adminNotes }) });
  revalidatePath("/admin/bookings");
  return { success: true };
}
